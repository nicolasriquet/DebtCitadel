...
from dojo.models import Debt_Notifications, Dojo_User, Alerts, UserContactInfo, System_Settings
from dojo.user.queries import get_authorized_users_for_debt_context_and_debt_context_type, get_authorized_users_for_debt_context_type
...
def create_notification(event=None, **kwargs):
    ...
    if 'recipients' in kwargs:
        ...
        for recipient_notifications in Debt_Notifications.objects.filter(user__username__in=kwargs['recipients'], user__is_active=True, debt_context=None):
        ...
        if 'debt_context_type' in kwargs:
            debt_context_type = kwargs.get('debt_context_type')
            logger.debug("Defined debt_context type %s", debt_context_type)

        debt_context = None
        if 'debt_context' in kwargs:
            debt_context = kwargs.get('debt_context')
            logger.debug("Defined debt_context  %s", debt_context)

        elif 'debt_engagement' in kwargs:
            debt_context = kwargs['debt_engagement'].debt_context
            logger.debug("Defined debt_context of debt_engagement %s", debt_context)

        elif 'debt_test' in kwargs:
            debt_context = kwargs['debt_test'].debt_engagement.debt_context
            logger.debug("Defined debt_context of debt_test %s", debt_context)

        elif 'debt_item' in kwargs:
            debt_context = kwargs['debt_item'].debt_test.debt_engagement.debt_context
            logger.debug("Defined debt_context of debt_item %s", debt_context)

        elif 'obj' in kwargs:
            from dojo.utils import get_debt_context
            debt_context = get_debt_context(kwargs['obj'])
            logger.debug("Defined debt_context of obj %s", debt_context)

        # System notifications
        try:
            system_notifications = Debt_Notifications.objects.get(user=None, template=False)
        except Exception:
            system_notifications = Debt_Notifications()
        ...
        # There are notification like deleting a debt_context type that shall not be sent to users.
        # These notifications will have the parameter no_users=True
        if not ('no_users' in kwargs and kwargs['no_users'] is True):
            # get users with either global notifications, or a debt_context specific noditiciation
            ...
            users = Dojo_User.objects.filter(is_active=True).prefetch_related(Prefetch(
                "notifications_set",
                queryset=Debt_Notifications.objects.filter(Q(debt_context_id=debt_context) | Q(debt_context__isnull=True)),
                to_attr="applicable_notifications"
            )).annotate(applicable_notifications_count=Count('debt_notifications__id', filter=Q(debt_notifications__debt_context_id=debt_context) | Q(debt_notifications__debt_context__isnull=True)))\
                .filter((Q(applicable_notifications_count__gt=0) | Q(is_superuser=True)))

            # only send to authorized users or admin/superusers
            logger.debug('Filtering users for the debt_context %s', debt_context)

            if debt_context:
                users = get_authorized_users_for_debt_context_and_debt_context_type(users, debt_context, Permissions.Debt_Context_View)

            elif debt_context_type:
                users = get_authorized_users_for_debt_context_type(users, debt_context_type, Permissions.Debt_Context_Type_View)
            else:
                # nor debt_context_type nor debt_context defined, we should not make noise and send only notifications to admins
                logger.debug('Debt_Context is not specified, making it silent')
                users = users.filter(is_superuser=True)

            for user in users:
                logger.debug("Authorized user for the debt_context %s", user)
                # send notifications to user after merging possible multiple notifications records (i.e. personal global + personal debt_context)
                ...
                notifications_set = Debt_Notifications.merge_notifications_list(applicable_notifications)
...
def create_description(event, *args, **kwargs):
    if "description" not in kwargs.keys():
        if event == 'debt_context_added':
            kwargs["description"] = _('Debt_Context %(title)s has been created successfully.' % {'title': kwargs['title']})
        elif event == 'debt_context_type_added':
            kwargs["description"] = _('Debt_Context Type %(title)s has been created successfully.' % {'title': kwargs['title']})
        else:
...
def send_alert_notification(event, user=None, *args, **kwargs):
    ...
    try:
        ...
        alert = Alerts(
            ...
            icon=icon[:25],
            source=Debt_Notifications._meta.get_field(event).verbose_name.title()[:100]
        )
...
def notify_debt_test_created(debt_test):
    title = 'Debt_Test created for ' + str(debt_test.debt_engagement.debt_context) + ': ' + str(debt_test.debt_engagement.name) + ': ' + str(debt_test)
    create_notification(event='debt_test_added', title=title, debt_test=debt_test, debt_engagement=debt_test.debt_engagement, debt_context=debt_test.debt_engagement.debt_context,
                        url=reverse('view_debt_test', args=(debt_test.id,)))


def notify_scan_added(debt_test, updated_count, new_debt_items=[], debt_items_mitigated=[], debt_items_reactivated=[], debt_items_untouched=[]):
    logger.debug("Scan added notifications")

    new_debt_items = sorted(list(new_debt_items), key=lambda x: x.numerical_severity)
    debt_items_mitigated = sorted(list(debt_items_mitigated), key=lambda x: x.numerical_severity)
    debt_items_reactivated = sorted(list(debt_items_reactivated), key=lambda x: x.numerical_severity)
