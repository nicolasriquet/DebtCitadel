...
from dojo.models import Debt_Notifications
...
class SystemNotificationsView(View):
    def get_notifications(self, request: HttpRequest):
        try:
            notifications = Debt_Notifications.objects.get(user=None, debt_context__isnull=True, template=False)
        except Debt_Notifications.DoesNotExist:
            notifications = Debt_Notifications(user=None, template=False)
    ...
    def get_form(self, request: HttpRequest, notifications: Debt_Notifications):
    ...
    def get_initial_context(self, request: HttpRequest, notifications: Debt_Notifications, scope: str):
        return {
...
class PersonalNotificationsView(SystemNotificationsView):
    def get_notifications(self, request: HttpRequest):
        try:
            notifications = Debt_Notifications.objects.get(user=request.user, debt_context__isnull=True)
        except Debt_Notifications.DoesNotExist:
            notifications = Debt_Notifications(user=request.user)
        return notifications
...
class TemplateNotificationsView(SystemNotificationsView):
    def get_notifications(self, request: HttpRequest):
        try:
            notifications = Debt_Notifications.objects.get(template=True)
        except Debt_Notifications.DoesNotExist:
            notifications = Debt_Notifications(user=None, template=True)
        return notifications
