...
from dojo.models import Product_Type, Debt_Context_Type, Finding, Debt_Item, Product, Debt_Context, Engagement, \
    Debt_Engagement, Test, Debt_Test, Dojo_User, Endpoint, Risk_Acceptance
...
from dojo.utils import get_page_items, add_breadcrumb, get_system_setting, get_period_counts_legacy, Product_Tab, \
    Debt_Context_Tab, get_words_for_field
...
def debt_context_report(request, pid):
    debt_context = get_object_or_404(Debt_Context, id=pid)
    return generate_report(request, debt_context)
...
@user_is_authorized(Debt_Context, Permissions.Debt_Context_View, 'pid')
def debt_context_endpoint_report(request, pid):
    user = Dojo_User.objects.get(id=request.user.id)
    debt_context = get_object_or_404(Debt_Context.objects.all().prefetch_related('debt_engagement_set__test_set__test_type', 'debt_engagement_set__test_set__environment'), id=pid)
    endpoint_ids = Endpoint.objects.filter(debt_context=debt_context,
                                           debt_item__active=True,
                                           debt_item__verified=True,
                                           debt_item__false_p=False,
                                           debt_item__duplicate=False,
                                           debt_item__out_of_scope=False,
                                           ).values_list('id', flat=True)
    ...
    paged_endpoints = get_page_items(request, endpoints.qs, 25)
    report_format = request.GET.get('report_type', 'AsciiDoc')
    include_debt_item_notes = int(request.GET.get('include_debt_item_notes', 0))
    include_debt_item_images = int(request.GET.get('include_debt_item_images', 0))
    include_executive_summary = int(request.GET.get('include_executive_summary', 0))
    include_table_of_contents = int(request.GET.get('include_table_of_contents', 0))
    include_disclaimer = int(request.GET.get('include_disclaimer', 0))
    disclaimer = get_system_setting('disclaimer')
    if include_disclaimer and len(disclaimer) == 0:
        disclaimer = 'Please configure in System Settings.'
    generate = "_generate" in request.GET
    add_breadcrumb(parent=debt_context, title="Vulnerable Debt_Context Endpoints Report", top_level=False, request=request)
    report_form = ReportOptionsForm()

    template = "dojo/debt_context_endpoint_pdf_report.html"
    report_name = "Debt_Context Endpoint Report: " + str(debt_context)
    report_title = "Debt_Context Endpoint Report"
    report_subtitle = str(debt_context)
    report_info = "Generated By %s on %s" % (
        user.get_full_name(), (timezone.now().strftime("%m/%d/%Y %I:%M%p %Z")))

    try:
        start_date = Debt_Item.objects.filter(endpoints__in=endpoints.qs).order_by('date')[:1][0].date
    except:
        start_date = timezone.now()

    end_date = timezone.now()

    risk_acceptances = Risk_Acceptance.objects.filter(debt_engagement__test__debt_item__endpoints__in=endpoints.qs)

    accepted_debt_items = [debt_item for ra in risk_acceptances
                           for debt_item in ra.accepted_debt_items.filter(endpoints__in=endpoints.qs)]

    verified_debt_items = Debt_Item.objects.filter(endpoints__in=endpoints.qs,
                                                   date__range=[start_date, end_date],
                                                   false_p=False,
                                                   verified=True,
                                                   duplicate=False,
    ...
    open_debt_items = Debt_Item.objects.filter(endpoints__in=endpoints.qs,
                                               false_p=False,
                                               verified=True,
                                               duplicate=False,
                                               out_of_scope=False,
    ...
    closed_debt_items = Debt_Item.objects.filter(endpoints__in=endpoints.qs,
                                                 false_p=False,
                                                 verified=True,
                                                 duplicate=False,
                                                 out_of_scope=False,
        ...
        if report_format == 'AsciiDoc':
            return render(request,
                          ...
                          {'debt_context_type': None,
                           ...
                           'accepted_debt_items': accepted_debt_items,
                           'open_debt_items': open_debt_items,
                           'closed_debt_items': closed_debt_items,
                           'verified_debt_items': verified_debt_items,
                           'debt_engagement': None,
                           'test': None,
                           'endpoints': endpoints,
                           'endpoint': None,
                           'debt_items': None,
                           'include_debt_item_notes': include_debt_item_notes,
                           'include_debt_item_images': include_debt_item_images,
                           'include_executive_summary': include_executive_summary,
                           'include_table_of_contents': include_table_of_contents,
                           'include_disclaimer': include_disclaimer,
                           'disclaimer': disclaimer,
    ...
    debt_context_tab = Debt_Context_Tab(debt_context, "Debt_Context Endpoint Report", tab="endpoints")
    return render(request,
                  'dojo/request_endpoint_report.html',
                  {"endpoints": paged_endpoints,
                   "filtered": endpoints,
                   ...
                   'report_form': report_form,
                   "name": "Vulnerable Debt_Context Endpoints",
                   })
