...
from dojo.models import Dojo_Group_Member, Product_Member, Debt_Context_Member, Product_Type_Member, \
    Debt_Context_Type_Member, Product_Type_Group, Debt_Context_Type_Group, Dojo_User, \
    Product_Group, Debt_Context_Group
...
def get_authorized_users_for_debt_context_and_debt_context_type(users, debt_context, permission):
    if users is None:
        users = Dojo_User.objects.filter(is_active=True)

    roles = get_roles_for_permission(permission)
    debt_context_members = Debt_Context_Member.objects \
    ...
    group_members = Dojo_Group_Member.objects \
        ...
        Q(group__in=[ptg.group for ptg in debt_context_type_groups])) \
        .select_related('user')
    return users.filter(Q(id__in=[pm.user.id for pm in debt_context_members]) |
                        Q(id__in=[ptm.user.id for ptm in debt_context_type_members]) |
                        Q(id__in=[gm.user.id for gm in group_members]) |
                        Q(global_role__role__in=roles) |
                        Q(is_superuser=True))
